...
For big datasets or high number of imputations, performing multiple imputation with function `mice` from package `mice` [@Stef2011] might take a long time. As a solution, wrapper function `parlMICE` was created to enable the imputation procedure to be run in parallel. This is done by dividing the imputations over multiple cores (or CPUs), thus potentially speeding up the process. 
...
The function `parlMICE` depends on packages `parallel` and `mice`. For more information about running functions in parallel, we refer to the [`parallel` manual](https://stat.ethz.ch/R-manual/R-devel/library/parallel/doc/parallel.pdf) and the document [*How-to-go parallel in R*](http://gforge.se/2015/02/how-to-go-parallel-in-r-basics-tips/) by Max Gordon. We also like to thank Max Gordon for his useful suggestions on [parallelization of `mice`'s chains on `stackoverflow`](http://stackoverflow.com/questions/24040280/parallel-computation-of-multiple-imputation-by-using-mice-r-package).
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=16, fig.height=5, cache=TRUE}
require(gridExtra)
require(ggplot2)
...
available, logical cores, results to `m = (4 - 1) * 2 = 6` inline r `r (detectCores() -1) * 2` imputations. We can check this by evaluating the `m` shown in the `mids` object. 
